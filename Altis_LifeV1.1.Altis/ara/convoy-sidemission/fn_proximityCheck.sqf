...
while {!isNull _vehicle} do {
	sleep 1;
	_isNotAttackable = ((str playerSide) in (getArray(missionConfigFile >> "Maverick_ConvoySidemission" >> "Config" >> "SidesNotAttackable")));
	if ((player distance _vehicle < 7.5) && !_isNotAttackable) then {
